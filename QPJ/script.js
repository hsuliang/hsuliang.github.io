...
const publishQuizBtn = document.getElementById('publish-quiz-btn'); // NEW
const shareQuizModal = document.getElementById('share-quiz-modal'); // NEW
const shareQuizModalContent = document.getElementById('share-quiz-modal-content'); // NEW
const quizShareLink = document.getElementById('quiz-share-link'); // NEW
const copyQuizLinkBtn = document.getElementById('copy-quiz-link-btn'); // NEW
const closeShareModalBtn = document.getElementById('close-share-modal-btn'); // NEW
...
function populateVersionHistory() {
    ...
    const currentDisplayVersion = 'v7.8 即時測驗';
    ...
    const versionHistory = [
        {
            ...
            notes: [
                "【🚀 重大功能新增】",
                " - 新增「發布測驗」功能！現在可以將生成的題目轉換為一個獨立的測驗網頁。",
                " - 老師可以將產生的專屬連結分享給學生，直接進行線上測驗。",
                " - 測驗結束後會自動計分，提供即時回饋。",
            ]
        },
         {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    addSafeEventListener(copyQuizLinkBtn, 'click', async () => {
        if(quizShareLink && quizShareLink.value) {
            try {
                await navigator.clipboard.writeText(quizShareLink.value);
                showToast('連結已成功複製！', 'success');
            } catch (err) {
