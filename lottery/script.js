...
const handleLoadData = async () => { loadButton.disabled = true; updateStatus('正在處理名單...'); if (activeDataSource === 'cloud') { await loadFromCloud(); } else { loadFromCustomInput(); } };
...
const loadFromCloud = async () => { const sheetUrl = sheetUrlInput.value.trim(); const sheetName = sheetNameInput.value.trim(); const columnLetter = columnLetterInput.value.trim().toUpperCase(); if (!sheetUrl) { updateStatus('請輸入 Google 試算表網址！', true); loadButton.disabled = false; return; } const sheetIdMatch = sheetUrl.match(/spreadsheets\/d\/([a-zA-Z0-9-_]+)/); if (!sheetIdMatch || !sheetIdMatch[1]) { updateStatus('無法從網址中解析出試算表 ID，請確認網址是否正確。', true); loadButton.disabled = false; return; } const sheetId = sheetIdMatch[1]; updateStatus('正在從雲端讀取名單...'); try { const timestamp = new Date().getTime(); const csvUrl = `https://docs.google.com/spreadsheets/d/${sheetId}/gviz/tq?tqx=out:csv&sheet=${encodeURIComponent(sheetName)}&_=${timestamp}`; const proxyUrl = `https://api.allorigins.win/raw?url=${encodeURIComponent(csvUrl)}`; const response = await fetch(proxyUrl); if (!response.ok) throw new Error(`網路回應錯誤: ${response.statusText}`); const csvText = await response.text(); parseCsvData(csvText, columnLetter); if (participants.length > 0 && readAndValidatePrizes()) { updateStatus(`成功載入 ${participants.length} 位參與者！`, false); switchToLotteryView(); } else { if (participants.length === 0) updateStatus(`在 ${columnLetter} 欄找不到任何資料，請確認工作表與欄位名稱。`, true); loadButton.disabled = false; } } catch (error) { console.error('讀取試算表時發生錯誤:', error); updateStatus('讀取失敗！請檢查：1. 網址是否正確 2. 試算表共用權限是否設為「知道連結的任何人」 3. 若權限剛修改，請稍候再試。', true); loadButton.disabled = false; } };
...
const switchToLotteryView = () => { settingsSection.classList.add('hidden'); lotterySection.classList.remove('hidden'); const isSimpleMode = simpleDrawToggle.checked; if (isSimpleMode) { prizes = []; prizes.push({ name: '抽出名單', quantity: participants.length, winners: [] }); participantLabel.textContent = '未中籤人數'; endDrawBtn.classList.remove('hidden'); resetBtn.classList.add('hidden'); } else { participantLabel.textContent = '剩餘摸彩人數'; endDrawBtn.classList.add('hidden'); resetBtn.classList.remove('hidden'); } updateParticipantCount(); updatePrizeDisplay(); };
...
const generateShareLink = () => { const sheetUrl = sheetUrlInput.value.trim(); if (!sheetUrl) { updateStatus('請先輸入試算表網址！', true); return; } const params = new URLSearchParams({ sheetUrl: sheetUrl, sheetName: sheetNameInput.value.trim(), column: columnLetterInput.value.trim() }); const baseUrl = window.location.href.split('?')[0]; const fullUrl = `${baseUrl}?${params.toString()}`; shareLinkOutput.value = fullUrl; linkOutputContainer.classList.remove('hidden'); };
const copyShareLink = () => { shareLinkOutput.select(); shareLinkOutput.setSelectionRange(0, 99999); try { document.execCommand('copy'); copyLinkBtn.innerHTML = '✅'; setTimeout(() => { copyLinkBtn.innerHTML = '📋'; }, 2000); } catch (err) { console.error('複製失敗: ', err); copyLinkBtn.innerHTML = '❌'; setTimeout(() => { copyLinkBtn.innerHTML = '📋'; }, 2000); } window.getSelection().removeAllRanges(); };
...
const resetApp = () => { settingsSection.classList.remove('hidden'); lotterySection.classList.add('hidden'); resetBtn.classList.add('hidden'); participants = []; prizes = []; currentPrizeIndex = 0; winnerDisplay.textContent = '準備開始！'; winnerDisplay.classList.remove('winner-reveal'); winnersList.innerHTML = ''; winnersListContainer.classList.add('hidden'); mainDrawPanel.classList.remove('md:w-[70%]'); mainDrawPanel.classList.add('md:w-full'); exportCsvBtn.classList.add('hidden'); participantCountSpan.textContent = '0'; currentPrizeDisplay.textContent = ''; mainTitle.textContent = titleInput.value.trim() || 'ㄚ亮笑長的抽抽樂'; participantLabel.textContent = '剩餘摸彩人數'; endDrawBtn.classList.add('hidden'); drawButton.textContent = '開始摸彩'; drawButton.disabled = false; endDrawBtn.disabled = false; sheetUrlInput.value = ''; sheetNameInput.value = '工作表1'; columnLetterInput.value = 'A'; customListInput.value = ''; prizesContainer.innerHTML = ''; addPrizeRow('三獎', 3); addPrizeRow('二獎', 2); addPrizeRow('頭獎', 1); loadButton.disabled = false; updateStatus(''); linkOutputContainer.classList.add('hidden'); shareLinkOutput.value = ''; switchTab('custom'); };
